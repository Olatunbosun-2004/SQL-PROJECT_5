...
SELECT CUSTOMER.CUST_NAME,CUSTOMER.CUST_COUNTRY,CUSTOMER.OPENING_AMT,AGENTS.AGENT_NAME FROM CUSTOMER
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
ORDER BY OPENING_AMT DESC

---show the country of customer that has the highest order amount
...
---the customer that has the lowest order amount and the customer that has the highest grade, are they the same?
SELECT CUSTOMER.CUST_NAME,CUSTOMER.GRADE,ORDERS.ORD_AMOUNT FROM CUSTOMER
LEFT JOIN ORDERS ON ORDERS.CUST_CODE = CUSTOMER.CUST_CODE
ORDER BY GRADE DESC

---show the phone number of the agent that brought the customer with the highest recieve amount
...
---what is the order number of all sales made on 2008-08-01 and show the information of the agents and the customer that is involved in this sales
SELECT ORDERS.ORD_NUM,ORDERS.ORD_DATE,AGENTS.AGENT_NAME,AGENTS.WORKING_AREA ,CUSTOMER.CUST_NAME,CUSTOMER.CUST_CODE FROM ORDERS
...
SELECT AGENTS.*,CUSTOMER.CUST_COUNTRY FROM AGENTS
LEFT JOIN CUSTOMER ON CUSTOMER.AGENT_CODE =AGENTS.AGENT_CODE
...
SELECT CUSTOMER.CUST_NAME,CUSTOMER.CUST_CITY,ORDERS.ORD_DATE,AGENTS.PHONE_NO FROM CUSTOMER
LEFT JOIN ORDERS ON ORDERS.CUST_CODE = CUSTOMER.CUST_CODE
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER. AGENT_CODE
WHERE CUST_NAME = 'Micheal' and CUST_CITY = 'Torento' 
---WHERE CUST_NAME = 'Martin' and CUST_CITY = 'Torento' 
